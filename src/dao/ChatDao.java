...
public class ChatDao {
    ...
    public ChatDao(int chatId, String chatType, String name, Timestamp createdAt) {
        this.chatId = chatId;
    ...
    public static ChatDao findById(Connection conn, int chatId) throws SQLException {
    ...
    public static List<ChatDao> getAllChats(Connection conn) throws SQLException {
        String query = "SELECT * FROM chat";
        List<ChatDao> chatDaos = new ArrayList<>();
        try (PreparedStatement stmt = conn.prepareStatement(query);
             ResultSet rs = stmt.executeQuery()) {
            while (rs.next()) {
                chatDaos.add(new ChatDao(
                        rs.getInt("chat_id"),
                        rs.getString("chat_type"),
                        rs.getString("name"),
                        rs.getTimestamp("created_at")
